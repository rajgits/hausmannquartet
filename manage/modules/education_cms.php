...
if (isset($_POST['submit']))
{
    ...
    {
        $chktitle=$db->getVal("select title from  education_cms where title='".trim($_POST['title'])."'");
        if($chktitle=='')
        {
            ...
            $flgIn = $db->insertAry(" education_cms", $aryData);        
            if (!is_null($flgIn))
            {
    ...
    {
        $chktitle=$db->getVal("select title from  education_cms where title='".trim($_POST['title'])."' && cms_id!='".$_GET['id']."'");
        if($chktitle=='')
        {
            $linkname= seo_url($_POST['title']);
            $aryData = array(
            ...
            if($_FILES["image"]["error"]==0)
            {
                ...
                if(in_array($ext,array('jpeg','jpg','gif','png')))
                {
                    $imgi = $db->getVal("select image from  education_cms where cms_id = '".$_GET['id']."'");
            ...
            $flgUp = $db->updateAry(" education_cms", $aryData, "where cms_id='".$_GET['id']."'");
            if (!is_null($flgUp))
            {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
...
elseif ($pgAct == "delete" && isset($_GET['id']) && trim($_GET['id']) != '') 
{
    ...
    @unlink("../uploads/cms/$imgi");
    @unlink("../uploads/cms/100x100/$imgi");
    $res=$db->delete(' education_cms',"where cms_id='".$_GET['id']."'"); 
    if(!is_null($res))
    {
...
<div class="row">
    <div class="col-md-12">  
        ...
        <ul class="page-breadcrumb breadcrumb">
            ...
            <!-- <li>Home Page Section-1</li>  -->
            <li><a href="<?php echo URL_ROOT; ?>/manage/?module=<?=$_GET['module'].'&section='.$_GET['section']?>">Education</a></li>  
        </ul>
...
if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) 
{
    ...
    {
        $aryForm = $db->getRow("SELECT * FROM  education_cms WHERE cms_id='" . $_GET['id'] . "'");
    }
                            ...
                            <form class="form-horizontal" enctype="multipart/form-data" method="post" id="myForm">
                                <div class="form-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-7">
                                        <input type='text' name="title" id="title" class="form-control" value='<?php if($_POST) { echo trim($_POST['title']);}else { echo $aryForm['title'];} ?>' placeholder="Enter title-1">
                                        </div>
                                    </div>
...
{
    $aryForm=$db->getRow("SELECT * FROM  education_cms WHERE cms_id='".(int)$_GET['id']."'");
    ?>
    <div class="portlet">
        <div class="portlet-title">
            <div class="caption">
            ...
            <form role="form" class="form-horizontal">
                <div class="form-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="control-label col-md-3">Description-1</label>
                                <div class="col-md-9">
                                    <p class="form-control-static">
...
{
    $sqlLimit = "SELECT * FROM   education_cms ORDER BY cms_id DESC";
