...
if (isset($_POST['submit']))
{
    ...
    {
        $chktitle=$db->getVal("select title from  press_cms where title='".trim($_POST['title'])."'");
        if($chktitle=='')
        {
            ...
            $flgIn = $db->insertAry(" press_cms", $aryData);        
            if (!is_null($flgIn))
            {
    ...
    {
        $chktitle=$db->getVal("select title from  press_cms where title='".trim($_POST['title'])."' && cms_id!='".$_GET['id']."'");
        if($chktitle=='')
        {
            $linkname= seo_url($_POST['title']);
            $aryData = array(
            ...
            if($_FILES["image"]["error"]==0)
            {
                ...
                if(in_array($ext,array('jpeg','jpg','gif','png')))
                {
                    $imgi = $db->getVal("select image from  press_cms where cms_id = '".$_GET['id']."'");
            ...
            $flgUp = $db->updateAry(" press_cms", $aryData, "where cms_id='".$_GET['id']."'");
            if (!is_null($flgUp))
            {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
...
elseif ($pgAct == "delete" && isset($_GET['id']) && trim($_GET['id']) != '') 
{
    ...
    @unlink("../uploads/cms/$imgi");
    @unlink("../uploads/cms/100x100/$imgi");
    $res=$db->delete(' press_cms',"where cms_id='".$_GET['id']."'"); 
    if(!is_null($res))
    {
...
if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) 
{
    ...
    {
        $aryForm = $db->getRow("SELECT * FROM  press_cms WHERE cms_id='" . $_GET['id'] . "'");
    }
                                ...
                                <div class="form-body">
                                   ...
                                   <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-7"> 
                                       <textarea id="orginal_post" rows='7' class="form-control tinymce"   name="orginal_post"><?php if($_POST) { echo trim($_POST['orginal_post']);}else { echo ucfirst(stripslashes($aryForm['orginal_post']));} ?></textarea>
                                        </div>
                                    </div> 
...
{
    $aryForm=$db->getRow("SELECT * FROM  press_cms WHERE cms_id='".(int)$_GET['id']."'");
    ?>
    <div class="portlet">
        <div class="portlet-title">
            <div class="caption">
            ...
            <form role="form" class="form-horizontal">
                <div class="form-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="control-label col-md-3">Orginal Post :</label>
                                <div class="col-md-9">
                                    <p class="form-control-static">
                                    <?php echo ucfirst($aryForm['orginal_post']); ?>
                                    </p>
                                </div>
                            </div>
                        </div>
...
{
    $sqlLimit = "SELECT * FROM   press_cms ORDER BY cms_id DESC";
