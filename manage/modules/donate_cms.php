...
if (isset($_POST['submit']))
{
    ...
    {
        $chktitle=$db->getVal("select title from  donate_cms where title='".trim($_POST['title'])."'");
        if($chktitle=='')
        {
            ...
            $flgIn = $db->insertAry(" donate_cms", $aryData);        
            if (!is_null($flgIn))
            {
    ...
    {
        $chktitle=$db->getVal("select title from  donate_cms where title='".trim($_POST['title'])."' && cms_id!='".$_GET['id']."'");
        if($chktitle=='')
        {
            $linkname= seo_url($_POST['title']);
            $aryData = array(
            ...
            if($_FILES["image"]["error"]==0)
            {
                ...
                if(in_array($ext,array('jpeg','jpg','gif','png')))
                {
                    $imgi = $db->getVal("select image from  donate_cms where cms_id = '".$_GET['id']."'");
            ...
            $flgUp = $db->updateAry(" donate_cms", $aryData, "where cms_id='".$_GET['id']."'");
            if (!is_null($flgUp))
            {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
...
elseif ($pgAct == "delete" && isset($_GET['id']) && trim($_GET['id']) != '') 
{
    ...
    @unlink("../uploads/cms/$imgi");
    @unlink("../uploads/cms/100x100/$imgi");
    $res=$db->delete(' donate_cms',"where cms_id='".$_GET['id']."'"); 
    if(!is_null($res))
    {
...
if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) 
{
    ...
    {
        $aryForm = $db->getRow("SELECT * FROM  donate_cms WHERE cms_id='" . $_GET['id'] . "'");
    }
...
{
    $aryForm=$db->getRow("SELECT * FROM  donate_cms WHERE cms_id='".(int)$_GET['id']."'");
    ?>
    <div class="portlet">
        <div class="portlet-title">
            <div class="caption">
...
{
    $sqlLimit = "SELECT * FROM   donate_cms ORDER BY cms_id DESC";
